...
class String
  ...
  # @firstline_pre[Fixnum] - The number of characters to subtract from the first line
  #   (used to allow for a line prefix external to the string).
  ...
  def fold!(fold_length = 75, firstline_pre = 0, fold_string = "\r\n ", fold_string_affect_size = 1)
    if (size + firstline_pre) > fold_length
      lines = []
      # The first line does not have a fold character on it, so treat it separately from the rest.
      lines << slice!(0, (fold_length - firstline_pre))
      # Split the string into fold-sized chunks.
      while size > (fold_length - fold_string_affect_size)
        lines << slice!(0, (fold_length - fold_string_affect_size))
      end
      # If there is a last bit left over from folding - add it as the last line.
      lines << self[0..-1] if size.positive?
