...
To generate the html report (with editor links if you’re using TextMate):
> rails_best_practices -f html --with-textmate --spec --features

To generate a text report in the terminal:
> rails_best_practices --spec --features

More info: http://rails-bestpractices.com

=== Metrical

Metrical generates a bunch of different reports and provides a common interface to them all.

To generate the metrical reports:
> metrical
That should automatically open up the reports index in your default web browser. If not, the index should be at ‘tmp/metric_fu/output/index.html’.
