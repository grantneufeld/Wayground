...
class Authority < ActiveRecord::Base
  ...
  scope :for_area, lambda {|area| where(:area => area)} # OR area = 'global'
  scope :for_item, lambda {|item| where(:item_id => item.id, :item_type => item.class.name)} # or area = #{item.area} or area = 'global'
