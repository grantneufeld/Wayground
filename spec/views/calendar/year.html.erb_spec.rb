  ...
  context 'with no user' do
    ...
    context 'with no events' do
      ...
      it 'should not render a link to the previous year' do
        render
        expect(rendered).not_to match(%r{<a [^>]*href="/calendar/2012"[^>]*>})
      end
      it 'should not render a link to the next year' do
        render
        expect(rendered).not_to match(%r{<a [^>]*href="/calendar/2014"[^>]*>})
      end
    end
    it 'should render the calendar heading' do
      render
      expect(rendered).to match(%r{<h1>.*2013.*</h1>})
    end
    ...
    it 'should render the months' do
      render
      expect(rendered).to match(%r{<a href="/calendar/2013/01"[^>]*>January</a>.* 1 event[^s]})
      expect(rendered).to match(%r{<a href="/calendar/2013/12"[^>]*>December</a>.* 24 events})
    end
  end
