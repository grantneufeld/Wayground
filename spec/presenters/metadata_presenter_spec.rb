  ...
  describe 'initialization' do
    ...
    context 'with an Image' do
      before(:all) do
        @image = Image.new
        # scaled
        @image.image_variants.new(url: 'http://s.tld/', format: 'png', height: 1, width: 2, style: 'scaled')
        # original
        @image.image_variants.new(
          url: 'http://o.tld/', format: 'png', height: 3, width: 6, style: 'original'
        )
        # another
        @image.image_variants.new(url: 'http://a.tld/', format: 'png', height: 2, width: 4, style: 'scaled')
        @image.save!
      end
      before(:each) do
  ...
  describe '#present_image' do
    ...
    context 'with an image url and size' do
      it 'should be an image, height and width, meta tags' do
        presenter = MetadataPresenter.new(
          view: view_stub, image_url: 'http://image.tld/', image_height: 12, image_width: 34
        )
        expect(presenter.present_image).to eq(
          "<meta property=\"og:image\" content=\"http://image.tld/\" />\n" \
          "<meta property=\"og:image:height\" content=\"12\" />\n" \
