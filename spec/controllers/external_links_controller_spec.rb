...
describe ExternalLinksController, type: :controller do
  ...
  describe "POST 'create'" do
    ...
    context 'with valid params' do
      it 'creates a new Event' do
        set_logged_in_admin
        expect { post :create, params: { event_id: event.id, external_link: valid_attributes } }.to change(
          event.external_links, :count
        ).by(1)
      end
  ...
  describe "PUT 'update'" do
    it 'requires the user to have authority' do
      ...
      patch(
        :update,
        params: { event_id: event.id, id: external_link.id, external_link: { 'these' => 'params' } }
      )
      expect(response.status).to eq 403
    end
    ...
    describe 'with valid params' do
      it 'updates the requested external_link' do
        ...
        patch(
          :update,
          params: { event_id: event.id, id: external_link.id, external_link: { 'title' => 'valid_params' } }
        )
      end
    ...
    describe 'with invalid params' do
      it 'assigns the external_link as @external_link' do
        ...
        patch(
          :update,
          params: { event_id: event.id, id: external_link.id, external_link: { url: 'invalid url' } }
        )
        expect(assigns(:external_link)).to eq(external_link)
      end
  ...
  describe "DELETE 'destroy'" do
    ...
    it 'destroys the requested external_link' do
      ...
      expect { delete :destroy, params: { event_id: event.id, id: delete_this.id } }.to change(
        event.external_links, :count
      ).by(-1)
    end
