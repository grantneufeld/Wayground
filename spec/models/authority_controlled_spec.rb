...
describe 'authority_controlled extensions to ApplicationRecord' do
  ...
  describe '#authority_restricted?' do
    it 'should be false for ActiveRecord models that are not set as authority_controlled' do
      expect(NO_AUTHORITY_CLASS.new.authority_restricted?).to be_falsey
    end
  end
  describe '#authority_for_user_to?' do
    it 'should allow viewing for models that are not set as authority_controlled' do
      expect(NO_AUTHORITY_CLASS.new.authority_for_user_to?).to be_truthy
    end
    it 'should not allow unauthorized users to change models that are not set as authority_controlled' do
      expect(NO_AUTHORITY_CLASS.new.authority_for_user_to?(nil, :can_update)).to be_falsey
    end
    it 'should refer to authority_area authorities for change actions when not set as authority_controlled' do
      ...
      FactoryGirl.create(:authority, user: user, area: 'Authority', can_update: true)
    ...
    it 'should fall back to global authorities for change actions when not set as authority_controlled' do
      item = FactoryGirl.create(:authority, area: 'test')
      user = FactoryGirl.create(:user)
      FactoryGirl.create(:authority, user: user, area: 'global', can_update: true)
      expect(item.authority_for_user_to?(user, :can_update)).to be_truthy
    end
  end
...
describe 'acts_as_authority_controlled' do
  describe 'item_authority_flag_field' do
    it 'should, when field is set to false, require authority for all records to be viewed' do
      expect(PRIVATE_AUTHORITY_CLASS.new.authority_restricted?).to be_truthy
    end
    it 'should, when using the default, restrict viewing records when the flag is set' do
      expect(SELECTIVE_AUTHORITY_CLASS.new(is_authority_controlled: true).authority_restricted?).to be_truthy
    end
    it 'should, when using the default, allow viewing records when the flag is not set' do
      expect(SELECTIVE_AUTHORITY_CLASS.new.authority_restricted?).to be_falsey
    end
...
describe 'authority_controlled class' do
  ...
  describe '#authority_restricted?' do
    ...
    it 'should be true for models that are set as authority_controlled and item is flagged' do
      ...
      expect(item.authority_restricted?).to be_truthy
    end
  end
  describe '#set_authority_for!' do
    it 'should assign access to a user for an item' do
      ...
      expect(item.authority_for_user_to?(new_accessor, :can_view)).to be_falsey
      item.set_authority_for!(new_accessor, :can_view)
      expect(item.authority_for_user_to?(new_accessor, :can_view)).to be_truthy
    end
    it 'should extend an existing authority' do
      ...
      authority = item.authority_for_user_to?(user, :can_delete)
  ...
  describe '#authority_for_user_to?' do
    ...
    it 'should have authority for the owner' do
      expect(@item.authority_for_user_to?(@owner, :can_update)).to be_truthy
    end
    it 'should have authority for a viewer' do
      expect(@item.authority_for_user_to?(@viewer)).to be_truthy
    end
    it 'should not allow a viewer to update' do
      expect(@item.authority_for_user_to?(@viewer, :can_update)).to be_falsey
    end
    it 'should not allow an unauthorized user' do
      unauthorized = FactoryGirl.create(:user)
      expect(@item.authority_for_user_to?(unauthorized)).to be_falsey
    end
  end
end
...
describe 'inherited authority model' do
  describe '#set_authority_for!' do
    it 'should raise an error' do
      expect { INHERITED_AUTHORITY_CLASS.new.set_authority_for!(User.new, :can_view) }.to raise_exception(
        Wayground::WrongModelForSettingAuthority
      )
    end
  end
  describe '#authority_for_user_to?' do
    ...
    it 'should allow area-authorized users access when no item to inherit from' do
      ...
      expect(item.authority_for_user_to?(user, :can_update)).to be_truthy
      expect(item.authority_for_user_to?(user, :can_delete)).to be_falsey
    end
  end
end
