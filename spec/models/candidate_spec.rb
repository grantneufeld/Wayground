...
describe Candidate, type: :model do
  ...
  describe 'validations' do
    ...
    describe 'of person' do
      ...
      it 'should fail if there is already a candidate for the person for the ballot' do
        ...
        # existing_candidate
        FactoryGirl.create(:candidate, ballot: @ballot, person: person)
        candidate = @ballot.candidates.build(required)
        candidate.person = person
        expect(candidate.valid?).to be_falsey
      end
      it 'should validate if there is already a candidate for the person for a different ballot' do
        person = FactoryGirl.create(:person)
        # different_ballot_candidacy
        FactoryGirl.create(:candidate, person: person)
        candidate = @ballot.candidates.build(required)
        candidate.person = person
        expect(candidate.valid?).to be_truthy
      end
  ...
  describe 'scopes' do
    before(:all) do
      @ballot = FactoryGirl.create(:ballot)
      @scoped_candidate1 = FactoryGirl.create(
        :candidate, ballot: @ballot, name: 'DEF', vote_count: 345, quit_on: 1.month.ago.to_date
      )
      @scoped_candidate2 = FactoryGirl.create(
        :candidate, ballot: @ballot, name: 'GHI', vote_count: 123, filename: 'the_param'
      )
      ...
      it 'should return the election that matches the param' do
        ...
        unless election
          election = @level.elections.new(
            filename: 'the_param', name: 'Candidate From Param', end_on: '2002-03-04'
          )
          election.save!
        end
