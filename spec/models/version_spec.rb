...
describe Version do
  ...
  describe "default_scope" do
    it "should keep the versions in order from oldest to newest" do
      ...
      newest_version = FactoryGirl.create(:version, :item => @item, :edited_at => 1.week.from_now)
  ...
  describe ".current_versions" do
    it "should restrict searches to the current versions for each item" do
      item_version = FactoryGirl.create(:version, :item => @item, :edited_at => 70.days.from_now)
  ...
  describe "#current" do
    it "should return the most recent edit on the version’s item" do
      version = FactoryGirl.create(:version, :item => @item, :edited_at => 2.years.ago)
  ...
  describe "#is_current?" do
    ...
    it "should be true if this is the latest version" do
      @item.versions.delete_all
      version = FactoryGirl.create(:version, :item => @item, :edited_at => 1.year.from_now)
      # create an additional, earlier, version to make sure we’re ordering by date properly
      FactoryGirl.create(:version, :item => @item, :edited_at => 1.day.ago)
      @item.reload
      version.is_current?.should be_true
    end
    it "should be false if this is not the latest version" do
      FactoryGirl.create(:version, :item => @item, :edited_at => 2.weeks.ago)
      FactoryGirl.create(:version, :item => @item, :edited_at => 1.week.from_now)
      middle_version = FactoryGirl.create(:version, :item => @item, :edited_at => 6.days.from_now)
