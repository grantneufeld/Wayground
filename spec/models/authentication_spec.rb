...
describe Authentication do
  ...
  describe "#label" do
    it "should make a special case for Twitter" do
      Authentication.new(:provider => 'twitter', :uid => '123', :nickname => 'nick').label.should eq '@nick'
    end
    it "should use the nickname, if present" do
      Authentication.new(:provider => :test, :uid => '123', :name => 'A Name', :nickname => 'Nick').label.should eq 'Nick'
    end
    it "should use the name, if present and no nickname" do
      Authentication.new(:provider => :test, :uid => '123', :name => 'A Name').label.should eq 'A Name'
    end
    it "should fall back on the uid, if nothing else" do
      Authentication.new(:provider => :test, :uid => '123').label.should eq '123'
    end
  end
end
