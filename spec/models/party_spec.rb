  ...
  describe 'validations' do
    ...
    describe 'of filename' do
      ...
      it 'should fail if filename is a duplicate for the level' do
        expect( @level.parties.new(required.merge(filename: 'duplicate-on-level')).valid? ).to be_false
      end
      it 'should validate if filename is a duplicate, but not for the level' do
        expect( @level.parties.new(required.merge(filename: 'duplicate-on-other')).valid? ).to be_true
      end
    ...
    describe 'of name' do
      ...
      it 'should fail if name is a duplicate for the level' do
        expect( @level.parties.new(required.merge(name: 'Duplicate for level')).valid? ).to be_false
      end
    ...
    describe 'of abbrev' do
      ...
      it 'should fail if abbrev is a duplicate for the level' do
        expect( @level.parties.new(required.merge(abbrev: 'dupe')).valid? ).to be_false
      end
      it 'should validate if abbrev is a duplicate, but not for the level' do
        expect( @level.parties.new(required.merge(abbrev: 'dup2')).valid? ).to be_true
      end
    end
    describe 'of colour' do
      it 'should validate if colour is a blank value' do
        expect( @level.parties.new(required.merge(colour: '')).valid? ).to be_true
      end
      it 'should validate if colour is a hexadecimal value' do
        expect( @level.parties.new(required.merge(colour: '#ABCDEF')).valid? ).to be_true
      end
      it 'should validate if colour is a named colour value' do
        expect( @level.parties.new(required.merge(colour: 'purple')).valid? ).to be_true
      end
    ...
    describe 'of registered_on' do
      it 'should fail if registered_on is before established_on' do
        ...
        expect( @level.parties.new(params).valid? ).to be_false
      end
    end
