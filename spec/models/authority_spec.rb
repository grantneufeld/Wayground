...
describe Authority, type: :model do
  ...
  describe 'validations' do
    before(:each) do
      @valid_user = stub_model(User, id: 1, email: 'test+user@wayground.ca')
      @valid_attributes = {
        item_id: nil,
        item_type: nil,
        area: 'global',
  ...
  describe '.merge_into!' do
    ...
    it 'should logical-OR the boolean fields' do
      ...
      authority2 = FactoryGirl.create(
        :authority,
        user: user, item: FactoryGirl.create(:event), can_view: true, can_update: true
      )
    ...
    it 'should save the changes to the destination authority' do
      ...
      authority2 = FactoryGirl.create(
        :authority, user: user, item: FactoryGirl.create(:event), can_view: true
      )
      authority1.merge_into!(authority2)
    ...
    it 'should destroy the source authority' do
      ...
      authority1.merge_into!(authority2)
      expect { Authority.find(destroyed_id) }.to raise_error(ActiveRecord::RecordNotFound)
    end
